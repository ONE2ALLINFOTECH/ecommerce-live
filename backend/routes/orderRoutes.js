...
router.post('/create', protectCustomer, async (req, res) => {
  try {
    ...
    if (!shippingAddress || !shippingAddress.name || !shippingAddress.mobile || 
        !shippingAddress.pincode || !shippingAddress.locality || !shippingAddress.address || 
        !shippingAddress.city || !shippingAddress.state) {
      return res.status(400).json({ message: 'Please provide complete shipping address including locality' });
    }
    ...
    if (paymentMethod === 'online') {
      ...
      // Try to create Ekart shipment
      try {
        console.log('🚚 Creating Ekart shipment for COD order...');
        const ekartResponse = await EkartService.createShipment(order, order.shippingAddress, order.items);
        ...
        order.ekartLabelUrl = ekartResponse.label_url;
        await order.save();

        console.log('✅ Ekart shipment created:', ekartResponse.tracking_id);
      } catch (ekartError) {
        console.error('❌ Ekart shipment creation failed:', ekartError.message);
        // Order still created successfully, shipment can be created later
      }
...
// ADMIN: Manually retry shipment creation
router.post('/admin/retry-shipment/:orderId', async (req, res) => {
  try {
    const { orderId } = req.params;
    
    console.log('🔄 Admin retrying shipment for order:', orderId);
    ...
    if (!['confirmed', 'pending'].includes(order.orderStatus)) {
      return res.status(400).json({ 
        success: false,
        message: `Cannot create shipment for order with status: ${order.orderStatus}`
      });
    }
...
router.put('/cancel/:orderId', protectCustomer, async (req, res) => {
  try {
    ...
    if (order.orderStatus === 'shipped' || order.orderStatus === 'delivered') {
      return res.status(400).json({ message: 'Cannot cancel order. It has already been shipped or delivered.' });
    }
    ...
    if (order.ekartTrackingId) {
      try {
        ...
        console.error('❌ Ekart cancellation failed:', cancelError.message);
      }
    }
